...
def generate_barcode_image(barcode_str):
    ...
    try:
        ...
        # Set a known font (Arial)
        options = {"font_path": r"C:\Windows\Fonts\arial.ttf"}
...
def draw_single_label(c, x_offset_mm, y_offset_mm, w_label_mm, h_label_mm, barcode_path, details):
...
def generate_label_pdf(pdf_path, w_mm, h_mm, barcode_path, details):
    ...
    # For two-up layout if size is (100, 25)
    if (w_mm, h_mm) == (100, 25):
        i = 0
        while i < count:
...
# ------------------------------
# Excel Export (with Stock Count)
# ------------------------------
def update_excel(item_code, item_name, sale_price, purchase_price, company_name, generation_date, stock_count):
    ...
    if not os.path.exists(file_name):
        ...
        ws = wb.active
        ws.title = "Items"
        ws.append(["Item Code", "Item Name", "Sale Price", "Purchase Price", "Company", "Stock Count", "Generation Date"])
        wb.save(file_name)
    ...
    for row in ws.iter_rows(min_row=2, values_only=False):
        if row[0].value == item_code:
            ...
            row[5].value = stock_count
            row[6].value = generation_date
            found = True
            break
    if not found:
        ws.append([item_code, item_name, sale_price, purchase_price, company_name, stock_count, generation_date])
