...
class TemplateDialog(QDialog):
    ...
    def generate_preview(self):
        ...
        label_size = "Medium (100x25 mm)"  # Default preview size
        ...
        try:
            ...
            # Draw sample text (top center)
            font_bold = QFont(self.current_template["font_family"], self.current_template["font_size"])
            font_bold.setBold(True)
            painter.setFont(font_bold)
            painter.drawText(QRectF(0, 10, width, 30), Qt.AlignCenter, "Sample Item")

            # Draw sample barcode (center)
            barcode_height = 40 * self.current_template["barcode_scale"]
            painter.drawRect(width // 2 - 50, height // 2 - barcode_height // 2, 100, barcode_height)

            # Draw sample info (bottom)
            font_normal = QFont(self.current_template["font_family"], self.current_template["font_size"] - 2)
            painter.setFont(font_normal)

            info_lines = []
            if self.current_template["show_price"]:
                info_lines.append("Sale: 100")
            ...
            info_text = " | ".join(info_lines)
            if info_text:
                painter.drawText(QRectF(0, height - 30, width, 20), Qt.AlignCenter, info_text)
...
class BarcodeGeneratorUI(QWidget):
    ...
    def update_preview(self):
        ...
        try:
            ...
            painter.drawText(QRectF(0, 10, width, 30), Qt.AlignCenter, item_name)
            ...
            if self.current_template["show_price"]:
                ...
                if "." in sale_price:
                    sale_price = sale_price.rstrip("0").rstrip(".") if "." in sale_price else sale_price
