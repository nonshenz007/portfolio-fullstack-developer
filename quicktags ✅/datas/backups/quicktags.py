...
class QuickTagApp(QMainWindow):
    ...
    def print_label(self):
        ...
        try:
            ...
            if self.generate_row_label_image(items, temp_bmp):
                # Open in Paint with fallback print settings
                subprocess.run(['mspaint.exe', '/pt', temp_bmp])
                QMessageBox.information(self, "Success", "Label sent to Paint for printing.")
        except Exception as e:
            QMessageBox.critical(self, "Print Error", f"Failed to print label: {str(e)}")

    def print_all_labels(self):
        """Print all labels via Paint with correct layout"""
        try:
            bmp_dir = os.path.join(os.path.dirname(__file__), "bmp_queue")
            os.makedirs(bmp_dir, exist_ok=True)

            self.cursor.execute("SELECT item_name, barcode, sale_price, stock_quantity FROM items")
            items = []
            for item_name, barcode_value, sale_price, quantity in self.cursor.fetchall():
                item_data = {
                    'item_name': item_name,
                    'barcode': barcode_value,
                    'sale_price': sale_price
                }
                items.extend([item_data] * quantity)

            for i in range(0, len(items), 2):
                pair = items[i:i+2]
                temp_bmp = os.path.join(bmp_dir, f"label_{i}.bmp")
                self.generate_row_label_image(pair, temp_bmp)
                subprocess.run(['mspaint.exe', '/pt', temp_bmp])
                time.sleep(1)

            QMessageBox.information(self, "Success", f"Printed {len(items)} labels successfully!")
        except Exception as e:
            QMessageBox.critical(self, "Print Error", f"Failed to print labels: {str(e)}")
